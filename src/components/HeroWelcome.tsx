...
export const HeroWelcome: React.FC<HeroWelcomeProps> = ({
  ...
  return (
    <div className="max-w-4xl mx-auto py-6 sm:py-10 px-4 sm:px-6 text-center space-y-6 sm:space-y-7 animate-msg-in">
      {/* Central Modern MAZEN AI Emblem (+15% larger, tight glow) */}
      <div className="relative inline-flex items-center justify-center">
        {/* Glow halo */}
        <div className="absolute w-36 h-36 rounded-3xl bg-gradient-to-tr from-[#6C63FF]/30 via-[#8B5CF6]/25 to-[#4F46E5]/30 blur-2xl animate-pulse" />
        
        {/* MA Logo Container */}
        <div className="relative w-28 h-28 rounded-3xl bg-gradient-to-tr from-[#6C63FF] via-[#8B5CF6] to-[#4F46E5] p-0.5 shadow-2xl shadow-[#6C63FF]/35">
          <div className="w-full h-full bg-[#090a10] light:bg-[#ffffff] rounded-[22px] flex items-center justify-center">
            <span className="font-display font-black text-5xl bg-gradient-to-tr from-[#6C63FF] via-[#8B5CF6] to-[#4F46E5] bg-clip-text text-transparent tracking-tight">
              M
            </span>
          </div>
        </div>
      ...
      <div className="space-y-3 max-w-xl mx-auto">
        <h1 className="font-display text-3xl sm:text-4xl font-extrabold tracking-tight text-[#edeef2] light:text-slate-900 leading-tight">
          ...
          {QUICK_ACTIONS.map((action, idx) => {
            ...
            return (
              <button
                ...
                className="group flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-white/5 light:bg-slate-100/90 border border-white/10 light:border-slate-200 hover:border-[#6C63FF]/60 hover:bg-[#6C63FF]/15 text-xs font-semibold text-[#edeef2] light:text-slate-800 transition-all duration-200 shadow-sm hover:shadow-md hover:shadow-[#6C63FF]/15 active:scale-95"
              >
